...
using Cinemachine; //This is needed for the custom tag field they coded.
//END 3367 CHANGES ==================
...
{
    //Added by 3367 ==================
    public enum DetectionType { Tag, Layer };
    [Header("Custom Trigger")]
    [Tooltip("Select what type of detection you want the bullet collision event to look for. Remember to use the variables below for which option you choose!")]
    [SerializeField] private DetectionType detectionType;
    [Tooltip("[Only used if Detection type is set to Layer] Layer the collider will look for before activating your event.")]
    [SerializeField] private LayerMask layerToDetect;
    [Tooltip("[Only used if Detection type is set to Tag] Tag the collider will look for before activating your event. Case sensitive!")]
    [TagField]
    [SerializeField] private string tagToDetect;
    [Tooltip("Show a debug message of 'Hit' in console for quick debugging")]
    [SerializeField] private bool debugCustomEvent;
    [Tooltip("This will attempt to execture the event on any object that contains the projectile impact script.")]
    [SerializeField] private bool executeProjectileImpactScript = false;
    [Tooltip("Unity Event. Remember, your own scripts can have public methods that can be called as desired.")]
    [SerializeField] private UnityEvent m_BulletHitEvent;
    //END 3367 CHANGES ==================
    ...
    {
        //Added by 3367 =========================
        bool customHitEvent = false;

        switch(detectionType)
        {
            case DetectionType.Tag:
            ...
            default:
                if(((1<<collider.gameObject.layer) & layerToDetect) != 0)
                {
        ...
        if(customHitEvent)
        {
            ...
            if(executeProjectileImpactScript)
            {
                if (collider.gameObject.GetComponent<ProjectileImpact>())
                {
                    collider.gameObject.GetComponent<ProjectileImpact>().ExecuteEvent();
                }
            }
